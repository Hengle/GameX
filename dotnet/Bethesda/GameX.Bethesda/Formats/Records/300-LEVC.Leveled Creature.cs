        ...
        public override bool CreateField(BinaryReader r, BethesdaFormat format, string type, int dataSize)
        {
            if (format == BethesdaFormat.TES3)
                switch (type)
                {
                    ...
                    case "NNAM": NNAM = r.ReadSAndVerify<BYTEField>(dataSize); return true;
                    case "INDX": INDX = r.ReadSAndVerify<IN32Field>(dataSize); return true;
                    case "CNAM": CNAMs.Add(r.ReadSTRV(dataSize)); return true;
                    case "INTV": INTVs.Add(r.ReadSAndVerify<IN16Field>(dataSize)); return true;
