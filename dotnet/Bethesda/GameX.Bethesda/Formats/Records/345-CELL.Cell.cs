        ...
        public override bool CreateField(BinaryReader r, BethesdaFormat format, string type, int dataSize)
        {
            ...
            if (!InFRMR)
                switch (type)
                {
                    ...
                    case "XCLC": XCLC = r.ReadSAndVerify<XCLCField>(format == BethesdaFormat.TES3 ? 8 : dataSize); return true;
                    case "XCLL":
                    case "AMBI": XCLL = r.ReadSAndVerify<XCLLField>(dataSize); return true;
                    case "XCLW":
                    case "WHGT": XCLW = r.ReadSAndVerify<FLTVField>(dataSize); return true;
                    // TES3
                    case "NAM0": NAM0 = r.ReadSAndVerify<UI32Field>(dataSize); return true;
                    case "INTV": INTV = r.ReadINTV(dataSize); return true;
                    case "NAM5": NAM5 = r.ReadSAndVerify<CREFField>(dataSize); return true;
                    // TES4
                    case "XCLR":
                        XCLRs = new FMIDField<REGNRecord>[dataSize >> 2];
                        for (var i = 0; i < XCLRs.Length; i++) XCLRs[i] = new FMIDField<REGNRecord>(r, 4); return true;
                    case "XCMT": XCMT = r.ReadSAndVerify<BYTEField>(dataSize); return true;
            ...
            // Referenced Object Data Grouping
            else switch (type)
                {
                    ...
                    case "FRMR":
                        ...
                        _lastRef.FRMR = r.ReadSAndVerify<UI32Field>(dataSize); return true;
                    case "NAME": _lastRef.EDID = r.ReadSTRV(dataSize); return true;
                    case "XSCL": _lastRef.XSCL = r.ReadSAndVerify<FLTVField>(dataSize); return true;
                    case "DODT": _lastRef.DODT = r.ReadSAndVerify<RefObj.XYZAField>(dataSize); return true;
                    case "DNAM": _lastRef.DNAM = r.ReadSTRV(dataSize); return true;
                    case "FLTV": _lastRef.FLTV = r.ReadSAndVerify<FLTVField>(dataSize); return true;
                    case "KNAM": _lastRef.KNAM = r.ReadSTRV(dataSize); return true;
                    case "TNAM": _lastRef.TNAM = r.ReadSTRV(dataSize); return true;
                    case "UNAM": _lastRef.UNAM = r.ReadSAndVerify<BYTEField>(dataSize); return true;
                    case "ANAM": _lastRef.ANAM = r.ReadSTRV(dataSize); return true;
                    case "BNAM": _lastRef.BNAM = r.ReadSTRV(dataSize); return true;
                    case "INTV": _lastRef.INTV = r.ReadSAndVerify<IN32Field>(dataSize); return true;
                    case "NAM9": _lastRef.NAM9 = r.ReadSAndVerify<UI32Field>(dataSize); return true;
                    case "XSOL": _lastRef.XSOL = r.ReadSTRV(dataSize); return true;
                    case "DATA": _lastRef.DATA = r.ReadSAndVerify<RefObj.XYZAField>(dataSize); return true;
                    //
                    case "CNAM": _lastRef.CNAM = r.ReadSTRV(dataSize); return true;
                    case "NAM0": _lastRef.NAM0 = r.ReadSAndVerify<UI32Field>(dataSize); return true;
                    case "XCHG": _lastRef.XCHG = r.ReadSAndVerify<IN32Field>(dataSize); return true;
                    case "INDX": _lastRef.INDX = r.ReadSAndVerify<IN32Field>(dataSize); return true;
                    default: return false;
                }
        }
    }
